...
private:
	...
	float rotation;				// float to contain the roation of the ship.
	Vector2D velocity;			// float to contain the ships velocity.
	Vector2D pos;				// float to contain the ships position as a vector.
	PictureIndex image;			// variable that contains the image information.
	Game* pGame = nullptr;		// variable that initalises a pointer to a Game as nullptr.
	std::list<GameObject*> pObjects; // list type varaible to hold all of the objects in the game in single list.
	ObjectManager om;		//varaiable of type ObjectManager to be used to add objects to manage objects.
	GameTimer theTimer;		// varaible to get a store frame time.
	Game();                        // Constructor
	~Game();                       // Destructor
